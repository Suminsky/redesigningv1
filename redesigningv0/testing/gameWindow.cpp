...
TestGameWindow::TestGameWindow( HINSTANCE hInst_p ) :Window(hInst_p)
{
	...
	//Create(winRect, TEXT("className"), NULL, TEXT("title"), WS_VISIBLE, 0 );
}

void TestGameWindow::ResizeIfNeed()
{
...
LRESULT CALLBACK TestGameWindow::WndProcHandler( HWND hWnd_p, UINT Msg_p, WPARAM wParam_p, LPARAM lParam_p )
{
	switch( Msg_p ){
...
#pragma endregion mouse stuff
	...
	case WM_CLOSE:
		if(!m_swapChain.m_bWindowed)
			m_swapChain.SwitchFullscreenMode(); // otherwise debug outputs shows live states //TODO: should I move it to swap chain dctor?
