...
namespace dx{
	...
	class PixelShader{
		...
		typedef ID3D11PixelShader res;
		typedef ID3D11PixelShader* pRes;
	...
	public:
		...
		struct Description{
			...
			std::string szProgramName;			// Pixel shader name
		...
		// Creates...
		//------------------------------------------------------------------------
		static ID3D11PixelShader* Create( ID3D11Device * pDeviceRef_p, const CreationParams & params_p ){
			
			ID3D11PixelShader *pPixelShader;		

			if( FAILED(
				pDeviceRef_p->CreatePixelShader( params_p.pShaderBytes->GetBufferPointer(), params_p.pShaderBytes->GetBufferSize(), NULL, &pPixelShader )
				)){
					//params_p.pShaderBytes->Release();
					throw std::exception("render:dx create PS failed");
			}
